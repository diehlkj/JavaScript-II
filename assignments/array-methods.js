...
runners.forEach((runner) => {
  fullNames.unshift(`${runner.first_name} ${runner.last_name}`);
});
...
runners.map(runner => firstNamesAllCaps.unshift(runner.first_name.toUpperCase()));
...
runners.filter(runner => {
  if (runner.shirt_size == 'L') {
    runnersLargeSizeShirt.unshift(runner);
  }
});
...
runners.filter(runner => {
  if (runner.donation >= 200) {
    topDoners.unshift(`${runner.first_name} ${runner.last_name}: $${runner.donation}`);
  }
});
...
let emailList = [];
runners.filter(runner => {
    emailList.unshift(`${runner.email}`);
});
...
let allCompany = [];
runners.forEach((runner) => {
  allCompany.unshift(`${runner.company_name}`);
});
let companiesSet = new Set(allCompany);
let companies = [...companiesSet];
